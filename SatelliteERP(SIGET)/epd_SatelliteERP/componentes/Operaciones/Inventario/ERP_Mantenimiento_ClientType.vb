Public Class ERP_Mantenimiento_ClientType
    ...
    Private Sub Mantenimiento_Generic_Municipality_Load(sender As Object, e As EventArgs) Handles Me.Load
        If id_OutletCatalog <> 0 Then
            txtNameCustomerType.Text = Name_OutletCatalog
            txtSystemType.Text = TypeSystem
            GroupBox1.Text = Title
        End If
    End Sub

    Private Sub cmdSave_Click(sender As Object, e As EventArgs) Handles cmdSave.Click
        If txtNameCustomerType.Text = "" Then
            MessageBox.Show("No puede dejar campos vacíos", Application.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Error)
            txtNameCustomerType.Focus()
            Exit Sub
        End If
        If txtSystemType.Text = "" Then
            MessageBox.Show("No puede dejar campos vacíos", Application.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Error)
            txtSystemType.Focus()
            Exit Sub
        End If
        If id_OutletCatalog > 0 Then ' Es modificacion
            Dim resp As String = objDatos.ERP_Generic_SP_UpdateTypeCustomer(id_OutletCatalog, Id_Company_FK, txtSystemType.Text, txtNameCustomerType.Text)
            If resp = "OK" Then
            ...
            End If
        Else
            Dim resp As String = objDatos.ERP_Generic_SP_InsertTypeCustomer(Id_Company_FK, txtSystemType.Text, txtNameCustomerType.Text)
            If resp = "OK" Then
                DialogResult = DialogResult.OK
