...
Public Class Financiero_Procesos_Amortizacion
    ...
    Property Id_HeaderAmortizacionAproved As Integer
    Private OpcionesMenu As DataTable
    Property BanderaProceso As Boolean = False

    Property ShowAmortizacion As Boolean = False
        ...
        Try
            ...
            Else
                ...
                If Not BanderaProceso Then
                    FechaSeleccionada = DTP_DateFactura.Value.Date
                ...
                Try
                    ...
                    For Each Seleccion As DataGridViewRow In DGActivosFij.Rows
                        IDString += "'" & Seleccion.Cells(0).Value.ToString & "',"
                    Next
                    IDString = IDString.ToString.Substring(0, IDString.ToString.Length - 1)
                Catch ex As Exception

                End Try
                If Id_HeaderAmortizacionAproved > 0 Then
        ...
        Try
            ...
            Using objSQL As New Financiero_SQLServer
                If Not BanderaProceso Then
                    ...
                    Id_HeaderAmortizacionAproved = objSQL.ERP_Finance_Set_FixedAssetAmortizacionHeader(FechaSeleccionada, Descripcion, 0, 0, IDString)
                    'Using obj As New Generic_SQLServer
                    '    obj.ERP_GetAmortizacionCalcInsert(FechaSeleccionada.ToString("yyyy-MM-dd"), IDString, Id_HeaderAmortizacionAproved)
                    'End Using
                    If Id_HeaderAmortizacionAproved > 0 Then
                        ...
                        'Close()
                    Else
                        MessageBox.Show("Error al generar la Amortización", Application.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Information)
                    End If
                    Exit Sub
                Else
                    If Id_HeaderAmortizacionAproved > 0 Then
                        Id_HeaderAmortizacionAproved = objSQL.ERP_Finance_Set_FixedAssetAmortizacionHeader(FechaSeleccionada, Descripcion, 1, Id_HeaderAmortizacionAproved, IDString)
                    End If
    ...
    Private Sub Financiero_ProcesosDepreciacion_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        If (Id_HeaderAmortizacionAproved > 0) Then
            Using objSQL As New Generic_SQLServer
                Dim dataInfo As DataTable = objSQL.ERP_GetReportAmortizacionFixedAssetPreviewData(_ID_Company, Id_HeaderAmortizacionAproved)
...
#End Region
    ...
    Private Sub BackgroundWorker2_DoWork(sender As Object, e As System.ComponentModel.DoWorkEventArgs) Handles BackgroundWorker2.DoWork
        ...
        Using objSQL As New Generic_SQLServer
            ...
            If (BanderaDepre = True) Then
                If (Id_HeaderAmortizacionAproved > 0) Then
                    Dim una As String = ""
                    _DT_DataReporte = objSQL.ERP_GetAmortizacionCalc(FechasString, IDString)
                Else
                    _DT_DataReporte = objSQL.ERP_GetAmortizacionCalc(FechasString, IDString)
                End If
            Else
                Dim dos As String = ""
                '_DT_DataReporte = objSQL.ERP_GetReportAmortizacionFixedAssetPreview(_ID_Company, FechaProcesoPreview, Id_HeaderAmortizacionAproved)
            End If

            Try
                If _DT_DataReporte.Rows.Count > 0 Then
                    Using ObjReport As New ERP_PreviewAmortizacion
    ...
    Private Sub BtnProcRechazar_Click(sender As Object, e As EventArgs) Handles BtnProcRechazar.Click
        ...
        Using obj As New Financiero_Procesos_DepreciacionRechazo
            If obj.ShowDialog() = DialogResult.OK Then
                ...
                Using objSQL As New Financiero_SQLServer
                    Id_HeaderAmortizacionAproved = objSQL.ERP_Finance_Set_FixedAssetAmortizacionHeader(Date.Now(), obj.Observacion, 2, Id_HeaderAmortizacionAproved, "")
                End Using
                Me.Close()
    ...
    Private Sub GetDataActivoFijo(GridProcess As DataGridView)
        ...
        _tableFixedAssetsMaster = objSql.GetAmortizacion_FixedAssetsMaster_Select(Data_Id_Company, "", Id_Department, 0, idemployee)
        Dim tablaCalc As New DataTable("Data")

        Dim con As Integer = 0
        Try
            tablaCalc = _tableFixedAssetsMaster
    ...
    Private Sub GetDataActivoFijo(ByVal busqueda As String, GridProcess As DataGridView)
        ...
        _tableFixedAssetsMaster = objSql.GetAmortizacion_FixedAssetsMaster_Select(Data_Id_Company, busqueda, Id_Department, 0, idemployee)
        Dim tablaCalc As New DataTable("Data")
