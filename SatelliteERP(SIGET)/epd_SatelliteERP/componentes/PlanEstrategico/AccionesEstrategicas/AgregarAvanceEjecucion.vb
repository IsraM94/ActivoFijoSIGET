Public Class AgregarAvanceEjecucion

#Region "Properties"
    Private objsql As New POA_SQLServer
    Private dtManagement As New DataTable
    Property Accion As String = ""
    Property IdAccionPOA As Integer = 0
    Property Anio As Integer = 0
    Property Mes As Integer = 0
    Property Documentos As Boolean = False
    Property MesString As String = ""
    Private RutaArchivo As String = ""
    Private NombreArchivo As String = ""
    Property _TipoAccion As Integer = 0
    Enum funcion
        Actualizar
        Agregar
    End Enum
...
#End Region
    ...
    Private Sub Config_InsertUserAccesos_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        lblMes.Text = "Avance del Mes de " & MesString
        ...
        If Documentos Then
            ...
            Try
                ...
                Using objDatos As New POA_SQLServer
                    dt3 = objDatos.ERP_StrategicPlan_GetAccionesArchivos(_Id_DptoPlani, IdAccionPOA, Anio, Mes)
                End Using

                If dt3.Rows.Count > 0 Then
                    For Each data As DataRow In dt3.Rows
                        txtObservacion.Text = data("ObservacionEje").ToString
                        txt_Avance.Text = data("Total").ToString
    ...
    Private Sub cmdGrabar_Click(sender As Object, e As EventArgs) Handles btnGrabar.Click
        ...
        Try
            If txt_Avance.Text.ToString = "" Then
                MessageBox.Show("No puede dejar campos en blanco.", Application.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Error)
                txt_Avance.Focus()
                Exit Sub
            End If

            If Convert.ToInt32(txt_Avance.Text.ToString) < 0 Then
                MessageBox.Show("Debe de ingresar un Avance correcto.", Application.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Error)
                txt_Avance.Focus()
                Exit Sub
            End If
        Catch ex As Exception
            MessageBox.Show("Debe ingresar un porcentaje de Avance.", Application.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Error)
            txt_Avance.Focus()
            Exit Sub
        End Try

        If txt_Ruta.Text.ToString.Trim = "" Then
        ...
        Dim dt As String
        dt = objsql.ERP_StrategicPlan_ProcessEjecucion(Mes, IdAccionPOA, txt_Avance.Text, RutaArchivo, Anio, NombreArchivo, txtObservacion.Text.ToString, _TipoAccion)
    ...
    Private Sub txt_Avance_KeyPress(sender As Object, e As KeyPressEventArgs) Handles txt_Avance.KeyPress
